...
| Rate-limit keys are forgeable, defeating the limit | The caller's address comes from `X-Forwarded-For`, which is **client-settable**. Azure appends the real client address to whatever arrived rather than replacing it, so the **last** entry is the trustworthy one — the conventional "take the first entry" advice is exactly wrong here and would let any caller mint unlimited keys by rotating a fake leading value. Verified against the real deployment rather than assumed, and unit-tested for the spoofing case. |
